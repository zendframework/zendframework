    ...
    implements Adapter
{
     ...
     * Ec2 Instance 
     * 
     * @var Zend\Servic\Amazon\Ec2\Instance
     */
    protected $ec2;
    /**
     * Ec2 Image
     * 
     * @var Zend\Servic\Amazon\Ec2\Image
     */
    protected $ec2Image;
    /**
     * Ec2 Zone
     * 
     * @var Zend\Servic\Amazon\Ec2\AvailabilityZone
     */
    protected $ec2Zone;
    /**
     * Ec2 Monitor 
     * 
     * @var Zend\Servic\Amazon\Ec2\CloudWatch
     */
     ...
     *
     * @return Zend\Cloud\Infrastructure\InstanceList|boolean
     */ 
    ...
    public function statusInstance($id)
    {
        ...
        return $this->map_status[$result['instanteState']['name']];
    }
    /**
     ...
     * @param string $name
     * @param array $options
     * @return Zend\Cloud\Infrastructure\Instance|boolean
     */ 
    ...
    public function monitorInstance($id,$metric,$options=null)
    {
        ...
        if (!in_array($metric,$this->validMetrics)) {
            throw new Exception\InvalidArgumentException("The metric $metric is not valid");
        }
        if (!empty($options) && !is_array($options)) {
            throw new Exception\InvalidArgumentException("The options must be an array");
        }
        if (!empty($options) && (empty($options[Instance::MONITOR_START_TIME]) || 
                                 empty($options[Instance::MONITOR_END_TIME]))) {
            throw new Exception\InvalidArgumentException('The options array must contains: '.$options[Instance::MONITOR_START_TIME].' and '.$options[Instance::MONITOR_END_TIME]);                  
        }      
        if (!isset($this->ec2Monitor)) {
            $this->ec2Monitor= new Ec2Monitor($this->accessKey,$this->accessSecret,$this->region);
        }
        $param= array('MeasureName' => $this->map_metrics[$metric],
    ...
    public function deployInstance($id,$params,$cmd)
    {
        if (!function_exists("ssh2_connect")) {
            throw new Exception\RuntimeException('You cannot deploy without the SSH extension for PHP');
        }
        if (empty($id)) {
        ...
        if (empty($params) || empty($params[Instance::SSH_USERNAME]) || (empty($params[Instance::SSH_PASSWORD]) &&
        ...
        if (empty($host)) {
            throw new Exception\RuntimeException("The instance $id specified doesn't exist.");
        }
        $conn= ssh2_connect($host);
        if (!ssh2_auth_password($conn, $params[Instance::SSH_USERNAME], $params[Instance::SSH_PASSWORD])) {
            throw new Exception\RuntimeException("SSH Authorization failed.");
        }
        if (is_array($cmd)) {
            $result=array();
     ...
     * Get the adapter 
     * 
     * @return Zend\Service\Amazon\Ec2\Instance 
     */
