 ...
 * @uses       \Zend\DB\Adapter\Oracle\Exception
 * @uses       \Zend\DB\Statement\Oracle\Oracle
 * @uses       \Zend\Loader
 * @category   Zend
 * @package    Zend_Db
 * @subpackage Adapter
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Oracle extends DB\Adapter\AbstractAdapter
{
    /**
    ...
    protected function _connect()
    {
        ...
        if (!extension_loaded('oci8')) {
            throw new Exception('The OCI8 extension is required for this adapter but the extension is not loaded');
        }
    ...
    protected function _commit()
    {
        if (!oci_commit($this->_connection)) {
            throw new Exception(oci_error($this->_connection));
        }
        $this->_setExecuteMode(OCI_COMMIT_ON_SUCCESS);
    }
    ...
    private function _setExecuteMode($mode)
    {
        switch($mode) {
            ...
            case OCI_DESCRIBE_ONLY:
                ...
                break;
            default:
                throw new Exception("Invalid execution mode '$mode' specified");
                break;
        }
    }
