 ...
 * @uses       \Zend\DB\Adapter\SQLSRV\Exception
 * @uses       \Zend\DB\Statement\SQLSRV\SQLSRV
 * @uses       \Zend\Loader
 * @category   Zend
 * @package    Zend_Db
 * @subpackage Adapter
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SQLSRV extends Adapter\AbstractAdapter
{
    /**
     ...
     *
     * @return void
     * @throws \Zend\DB\Adapter\SQLSRV\Exception
     */
    ...
    protected function _checkRequiredOptions(array $config)
    {
        ...
        if (! array_key_exists('password', $config) && array_key_exists('username', $config)) {
            throw new Adapter\Exception("Configuration array must have a key for 'password' for login credentials.
                                                If Windows Authentication is desired, both keys 'username' and 'password' should be ommited from config.");
        }

        if (array_key_exists('password', $config) && !array_key_exists('username', $config)) {
            throw new Adapter\Exception("Configuration array must have a key for 'username' for login credentials.
                                                If Windows Authentication is desired, both keys 'username' and 'password' should be ommited from config.");
        }
    }
    ...
    public function setTransactionIsolationLevel($level = null)
    {
        ...
        switch ($level) {
            ...
            case SQLSRV_TXN_SERIALIZABLE:
                ...
                break;
            default:
                throw new Exception("Invalid transaction isolation level mode '$level' specified");
        }

        if (!sqlsrv_query($this->_connection, "SET TRANSACTION ISOLATION LEVEL $sql;")) {
            throw new Exception("Transaction cannot be changed to '$level'");
        }
     ...
     * @return \Zend\DB\Statement\SQLSRV\SQLSRV
     */
    ...
    public function setFetchMode($mode)
    {
        switch ($mode) {
            ...
            case DB\DB::FETCH_BOUND: // bound to PHP variable
                throw new Exception('FETCH_BOUND is not supported yet');
                break;
            default:
