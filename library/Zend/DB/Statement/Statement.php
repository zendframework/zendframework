...
abstract class Statement implements StatementInterface
{
     ...
     * @param mixed $sql Either a string or \Zend\DB\Select\Select.
     */
    public function __construct($adapter, $sql)
    {
        $this->_adapter = $adapter;
        if ($sql instanceof DB\Select\Select) {
            $sql = $sql->assemble();
        }
    ...
    {
        $data = array();
        if ($style === DB\DB::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        $data = array();
        $col = (int) $col;
        $row = $this->fetch(DB\DB::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(DB\DB::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
