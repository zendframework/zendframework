 ...
 * @uses       \Zend\DB\Table\Row\Exception
 * @uses       \Zend\Loader
 * @category   Zend
 * @package    Zend_Db
 * @subpackage Table
...
abstract class AbstractRow implements \ArrayAccess
{
    ...
    {
        if (isset($config['table']) && $config['table'] instanceof Table\AbstractTable) {
     ...
     * @throws \Zend\DB\Table\Row\Exception if the $columnName is not a string.
     */
     ...
     * @throws \Zend\DB\Table\Row\Exception if the $columnName is not a column in the row.
     */
    ...
    protected function _getWhereQuery($useDirty = true)
    {
        ...
        $metadata = $info[Table\AbstractTable::METADATA];

        // retrieve recently updated row using primary keys
        $where = array();
        foreach ($primaryKey as $column => $value) {
            $tableName = $db->quoteIdentifier($info[Table\AbstractTable::NAME], true);
     ...
     * @return array
     */
    protected function _prepareReference(Table\AbstractTable $dependentTable, Table\AbstractTable $parentTable, $ruleKey)
    {
        ...
        if (!isset($map[Table\AbstractTable::REF_COLUMNS])) {
            $parentInfo = $parentTable->info();
            $map[Table\AbstractTable::REF_COLUMNS] = array_values((array) $parentInfo['primary']);
        }

        $map[Table\AbstractTable::COLUMNS] = (array) $map[Table\AbstractTable::COLUMNS];
        $map[Table\AbstractTable::REF_COLUMNS] = (array) $map[Table\AbstractTable::REF_COLUMNS];
     ...
     *
     * @param string|\Zend\DB\Table\AbstractTable  $dependentTable
     * @param string                         OPTIONAL $ruleKey
     * @param \Zend\DB\Table\Select\Select           OPTIONAL $select
     * @return \Zend\DB\Table\Rowset\AbstractRowset Query result from $dependentTable
     * @throws \Zend\DB\Table\Row\Exception If $dependentTable is not a table or is not loadable.
     */
    public function findDependentRowset($dependentTable, $ruleKey = null, Select\Select $select = null)
    {
        ...
        if (!$dependentTable instanceof Table\AbstractTable) {
            $type = gettype($dependentTable);
            if ($type == 'object') {
                $type = get_class($dependentTable);
            }
            throw new Exception("Dependent table must be a Zend_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $dependentTable->setOptions(array(Table\AbstractTable::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($map[Table\AbstractTable::COLUMNS]); ++$i) {
            $parentColumnName = $db->foldCase($map[Table\AbstractTable::REF_COLUMNS][$i]);
            ...
            $dependentColumnName = $dependentDb->foldCase($map[Table\AbstractTable::COLUMNS][$i]);
            $dependentColumn = $dependentDb->quoteIdentifier($dependentColumnName, true);
            $dependentInfo = $dependentTable->info();
            $type = $dependentInfo[Table\AbstractTable::METADATA][$dependentColumnName]['DATA_TYPE'];
     ...
     * @throws \Zend\DB\Table\Row\Exception If $parentTable is not a table or is not loadable.
     */
    public function findParentRow($parentTable, $ruleKey = null, Select\Select $select = null)
    {
        ...
        if (!$parentTable instanceof Table\AbstractTable) {
            $type = gettype($parentTable);
            if ($type == 'object') {
                $type = get_class($parentTable);
            }
            throw new Exception("Parent table must be a Zend_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $parentTable->setOptions(array(Table\AbstractTable::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($map[Table\AbstractTable::COLUMNS]); ++$i) {
            $dependentColumnName = $db->foldCase($map[Table\AbstractTable::COLUMNS][$i]);
            ...
            $parentColumnName = $parentDb->foldCase($map[Table\AbstractTable::REF_COLUMNS][$i]);
            $parentColumn = $parentDb->quoteIdentifier($parentColumnName, true);
            $parentInfo = $parentTable->info();

            // determine where part
            $type     = $parentInfo[Table\AbstractTable::METADATA][$parentColumnName]['DATA_TYPE'];
            $nullable = $parentInfo[Table\AbstractTable::METADATA][$parentColumnName]['NULLABLE'];
    ...
    /**
     * @param  string|\Zend\DB\Table\AbstractTable  $matchTable
     * @param  string|\Zend\DB\Table\AbstractTable  $intersectionTable
     * @param  string                         OPTIONAL $callerRefRule
     * @param  string                         OPTIONAL $matchRefRule
     * @param  \Zend\DB\Table\Select\Select           OPTIONAL $select
     * @return \Zend\DB\Table\Rowset\AbstractRowset Query result from $matchTable
     * @throws \Zend\DB\Table\Row\Exception If $matchTable or $intersectionTable is not a table class or is not loadable.
     */
                                         ...
                                         $matchRefRule = null, Select\Select $select = null)
    {
        ...
        if (!$intersectionTable instanceof Table\AbstractTable) {
            $type = gettype($intersectionTable);
            if ($type == 'object') {
                $type = get_class($intersectionTable);
            }
            throw new Exception("Intersection table must be a Zend_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $intersectionTable->setOptions(array(Table\AbstractTable::DEFINITION => $tableDefinition));
        }
        ...
        if (! $matchTable instanceof Table\AbstractTable) {
            $type = gettype($matchTable);
            if ($type == 'object') {
                $type = get_class($matchTable);
            }
            throw new Exception("Match table must be a Zend_Db_Table_Abstract, but it is $type");
        }
        ...
        if (($tableDefinition = $this->_table->getDefinition()) !== null
            ...
            $matchTable->setOptions(array(Table\AbstractTable::DEFINITION => $tableDefinition));
        }
        ...
        for ($i = 0; $i < count($matchMap[Table\AbstractTable::COLUMNS]); ++$i) {
            $interCol = $interDb->quoteIdentifier('i' . '.' . $matchMap[Table\AbstractTable::COLUMNS][$i], true);
            $matchCol = $interDb->quoteIdentifier('m' . '.' . $matchMap[Table\AbstractTable::REF_COLUMNS][$i], true);
            $joinCond[] = "$interCol = $matchCol";
        }
        $joinCond = implode(' AND ', $joinCond);

        $select->from(array('i' => $interName), array(), $interSchema)
               ->joinInner(array('m' => $matchName), $joinCond, \Zend\DB\Select\Select::SQL_WILDCARD, $matchSchema)
               ->setIntegrityCheck(false);

        $callerMap = $this->_prepareReference($intersectionTable, $this->_getTable(), $callerRefRule);

        for ($i = 0; $i < count($callerMap[Table\AbstractTable::COLUMNS]); ++$i) {
            $callerColumnName = $db->foldCase($callerMap[Table\AbstractTable::REF_COLUMNS][$i]);
            $value = $this->_data[$callerColumnName];
            $interColumnName = $interDb->foldCase($callerMap[Table\AbstractTable::COLUMNS][$i]);
            $interCol = $interDb->quoteIdentifier("i.$interColumnName", true);
            $interInfo = $intersectionTable->info();
            $type = $interInfo[Table\AbstractTable::METADATA][$interColumnName]['DATA_TYPE'];
...
//        }
     ...
     *
     * @param string $method
     * @param array $args OPTIONAL \Zend\DB\Table\Select\Select query modifier
     * @return Zend_Db_Table_Row_Abstract|\Zend\DB\Table\Rowset\AbstractRowset
     * @throws \Zend\DB\Table\Row\Exception If an invalid method is called.
     */
    public function __call($method, array $args)
    {
        $matches = array();

        if (count($args) && $args[0] instanceof Select\Select) {
            $select = $args[0];
        } else {
            $select = null;
        }
