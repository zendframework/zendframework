...
class DefaultIterator implements \Iterator, \Countable
{
    ...
    public function __construct(Ldap\Ldap $ldap, $resultId)
    {
        ...
        if ($this->itemCount === false) {
            throw new Ldap\Exception($this->ldap, 'counting entries');
        }
    }
    ...
    public function current()
    {
        ...
        $ber_identifier = null;
        $name = @ldap_first_attribute($this->ldap->getResource(), $this->current,
            $ber_identifier);
        while ($name) {
            ...
            $entry[$attrName] = $data;
            $name = @ldap_next_attribute($this->ldap->getResource(), $this->current,
                $ber_identifier);
        }
    ...
    public function key()
    {
        ...
        if (is_resource($this->current)) {
            ...
            if ($currentDn === false) {
                throw new Ldap\Exception($this->ldap, 'getting dn');
            }
            return $currentDn;
        } else {
            return null;
            ...
            if ($this->current === false) {
                ...
                if ($code === Ldap\Exception::LDAP_SIZELIMIT_EXCEEDED) {
                    ...
                    return;
                     ...
                     throw new Ldap\Exception($this->ldap, 'getting next entry (' . $msg . ')');
                }
            }
        } else {
            $this->current = false;
    ...
    public function rewind()
    {
        if (is_resource($this->resultId)) {
            ...
            if ($this->current === false &&
                    $this->ldap->getLastErrorCode() > Ldap\Exception::LDAP_SUCCESS) {
                throw new Ldap\Exception($this->ldap, 'getting first entry');
            }
        }
    }
