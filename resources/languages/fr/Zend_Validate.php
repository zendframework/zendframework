...
return array(
    // Zend_Validate_Alnum
    "Invalid type given, value should be float, string, or integer" => "Type de donnée non valide; entier, flottant ou chaine attendu",
    ...
    "'%value%' is an empty string" => "'%value%' est une chaine vide",

    // Zend_Validate_Alpha
    "Invalid type given, value should be a string" => "Type de donnée non valide; chaine attendue",
    ...
    "Invalid type given, value should be string" => "Type de donnée non valide; chaine attendue",
    ...
    "Invalid type given, value should be a string" => "Type invalide, chaine attendue",
    ...
    // Zend_Validate_Date
    "Invalid type given, value should be string, integer, array or Zend_Date" => "Type invalide; chaine, entier, tableau ou Zend_Date requis",
    ...
    "Invalid type given, value should be string, integer or float" => "Type invalide; chaine entier ou flottant attendu",
    ...
    "'%hostname%' is no valid hostname for email address '%value%'" => "'%hostname%' n'est pas un nom d'hôte valide pour l'addresse email '%value%'",
    ...
    "'%hostname%' is not in a routable network segment. The email address '%value%' should not be resolved from public network." => "'%hostname%' n'est pas dans un segment réseau routable. L'adresse email '%value%' ne devrait petre résolue depuis un réseau public.",
    ...
    "'%value%' exceeds the allowed length" => "'%value%' excède la taille requise",

    // Zend_Validate_File_Count
    "Too many files, maximum '%max%' are allowed but '%count%' are given" => "Trop de fichiers, '%max%' maximum sont autorisés mais '%count%' ont été donnés",
    "Too few files, minimum '%min%' are expected but '%count%' are given" => "Trop peu de fichiers, '%min%' minimum sont autorisés mais '%count%' ont été donnés",
    ...
    "All files in sum should have a maximum size of '%max%' but '%size%' were detected" => "Tous les fichiers devraient avoir une taille maximale de '%max%' mais '%size%' a été détectée",
    "All files in sum should have a minimum size of '%min%' but '%size%' were detected" => "Tous les fichiers devraient avoir une taille minimale de '%min%' mais '%size%' a été détectée",
    ...
    "Maximum allowed height for image '%value%' should be '%maxheight%' but '%height%' detected" => "La hauteur maximale de l'image '%value%' devrait être '%maxheight%' mais '%height%' a été détectéed",
    ...
    "File '%value%' does not match the given sha1 hashes" => "Le fichier '%value%' ne correspond pas à la somme de contrôle sha1",
    "A sha1 hash could not be evaluated for the given file" => "La valeur de somme de contrôle sha1 n'a pas pu être calculée pour le fichier",
    "File '%value%' could not be found" => "Fichier '%value%' introuvable",
    ...
    "A PHP extension returned an error while uploading the file '%value%'" => "Une extension PHP a retourné une erreur lors de l'envoi de '%value%'",
    ...
    "Too much words, maximum '%max%' are allowed but '%count%' were counted" => "Trop de mots, un maximum de '%max%' est requis, '%count%' ont été donnés",
    "Too less words, minimum '%min%' are expected but '%count%' were counted" => "Trop peu de mots, un minimum de '%min%' est requis, '%count%' ont été donnés",
    "File '%value%' could not be found" => "Le fichier '%value%' est introuvable",

    // Zend_Validate_Float
    "Invalid type given, value should be float, string, or integer" => "Type invalide; chaine, entier ou flottant attendu",
    ...
    // Zend_Validate_Hex
    "Invalid type given, value should be a string" => "Type de donnée non valide, chaine attendue",
    "'%value%' has not only hexadecimal digit characters" => "'%value%' ne contient pas uniquement des caractères hexadécimaux",

    // Zend_Validate_Hostname
    "Invalid type given, value should be a string" => "Type de donnée non valide, chaine attendue",
    "'%value%' appears to be an IP address, but IP addresses are not allowed" => "'%value%' semble être une IP valide mais celles-ci ne sont pas accéptées",
    ...
    "'%value%' does not match the expected structure for a DNS hostname" => "'%value%' ne correspond oas à la structure d'un nom d'hôte DNS valide",
    ...
    // Zend_Validate_Int
    "Invalid type given, value should be string or integer" => "Type invalide, chaine ou entier attendu",
    ...
    "'%value%' is not less than '%max%'" => "'%value%' ne fait pas moins que '%max%'",

    // Zend_Validate_NotEmpty
    "Invalid type given, value should be float, string, array, boolean or integer" => "Type invalide; chaine, entier tableau booléen ou flottant attendu",
    ...
    // Zend_Validate_Regex
    "Invalid type given, value should be string, integer or float" => "Type invalide, chaine entier ou flottant attendu",
    "'%value%' does not match against pattern '%pattern%'" => "'%value%' n'a pas de correspondance sur le motif '%pattern%'",
    ...
    "'%value%' is no valid sitemap location" => "'%value%' n'est pas un endroit valide pour une sitemap",

    // Zend_Validate_Sitemap_Priority
    "'%value%' is no valid sitemap priority" => "'%value%' n'est pas une priorité sitemap valide",

    // Zend_Validate_StringLength
    "Invalid type given, value should be a string" => "Type de donnée non valide, chaine de caractères attendue",
    "'%value%' is less than %min% characters long" => "'%value%' devrait faire %min% caractères minimum",
    "'%value%' is more than %max% characters long" => "'%value%' devrait faire %max% caractères maximum",
);
