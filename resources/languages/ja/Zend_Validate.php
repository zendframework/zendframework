...
return array(
    // Zend_Validate_Alnum
    "Invalid type given, value should be float, string, or integer" => "不正な形式です。小数、文字列もしくは数値である必要があります",
    ...
    "Failure within the callback, exception returned" => "コールバックに失敗しました。例外を応答しました",
    ...
    "Validation of '%value%' has been failed by the service" => "'%value%' の検証はサービスにより失敗しました",
    "The service returned a failure while validating '%value%'" => "'%value%' を検証中にサービスが失敗を応答しました",
    ...
    "File '%value%' could not be found" => "ファイル '%value%' は存在しません",
    ...
    "File '%value%' can not be read" => "ファイル '%value%' は読み込めませんでした",
