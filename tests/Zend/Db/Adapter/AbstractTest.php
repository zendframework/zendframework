...
abstract class AbstractTest extends \ZendTest\Db\TestSetup
{
    ...
    {
        $params = new Config\Config($this->_util->getParams());
    ...
    public function testAdapterZendConfigEmptyNamespace()
    {
        ...
        $params = new Config\Config($params);
    ...
    public function testAdapterAutoQuoteIdentifiersFalse()
    {
        ...
        $params['options'] = array(
            Db\Db::AUTO_QUOTE_IDENTIFIERS => false
        );
        $db = Db\Db::factory($this->getDriver(), $params);
        $db->getConnection();
    ...
    protected function _testAdapterConstructInvalidParam($param, $adapterClass = null)
    {
        ...
        try {
            ...
            $this->assertInstanceOf($exceptionClass, $e, "Expected to catch $exceptionClass, got ".get_class($e));
    ...
    public function testAdapterFetchAllOverrideFetchMode()
    {
        ...
        $result = $this->_db->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1, Db\Db::FETCH_ASSOC);
        $this->assertEquals(2, count($result));
        ...
        $result = $this->_db->fetchAll("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id ASC", 1, Db\Db::FETCH_BOTH);
        $this->assertEquals(2, count($result));
        $this->assertInternalType('array', $result[0]);
    ...
    public function testAdapterFetchRowOverrideFetchMode()
    {
        ...
        // Test associative array
        $result = $this->_db->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1, Db\Db::FETCH_ASSOC);
        $this->assertInternalType('array', $result);
        $this->assertEquals(2, count($result)); // count columns
        $this->assertEquals(2, $result['product_id']);

        // Test numeric and associative array
        $result = $this->_db->fetchRow("SELECT * FROM $products WHERE $product_id > ? ORDER BY $product_id", 1, Db\Db::FETCH_BOTH);
        $this->assertInternalType('array', $result);
    ...
    public function _testAdapterOptionCaseFoldingCommon($case)
    {
        ...
        $stmt = $db->query($sql);
        $result = $stmt->fetchAll(Db\Db::FETCH_ASSOC);
    ...
    {
        $natural = $this->_testAdapterOptionCaseFoldingCommon(Db\Db::CASE_NATURAL);
    ...
    {
        $upper = $this->_testAdapterOptionCaseFoldingCommon(Db\Db::CASE_UPPER);
    ...
    {
        $lower = $this->_testAdapterOptionCaseFoldingCommon(Db\Db::CASE_LOWER);
    ...
    public function testAdapterQueryAfterUnclosedQuery()
    {
        ...
        $result1 = $stmt1->fetchAll(Db\Db::FETCH_NUM);

        $stmt1 = $this->_db->query($select);
        $result2 = $stmt1->fetchAll(Db\Db::FETCH_NUM);
    ...
    public function testAdapterQueryResultsNone()
    {
        ...
        $this->assertInstanceOf('Zend\Db\Statement', $stmt,
    ...
    {
        $value = $this->_db->quoteInto('foo = ?', 1234, Db\Db::INT_TYPE);
        $this->assertInternalType('string', $value);
        $this->assertEquals('foo = 1234', $value,
            'Incorrect quoteInto() result for INT_TYPE');

        $value = $this->_db->quoteInto('foo = ?', 1234, Db\Db::BIGINT_TYPE);
        $this->assertInternalType('string', $value);
        $this->assertEquals('foo = 1234', $value,
            'Incorrect quoteInto() result for BIGINT_TYPE');

        $value = $this->_db->quoteInto('foo = ?', 12.34, Db\Db::FLOAT_TYPE);
        $this->assertInternalType('string', $value);
    ...
    public function testAdapterQuoteTypeInt()
    {
        foreach ($this->_numericDataTypes as $typeName => $type) {
            ...
            $value = $this->_db->quote(new Db\Expr('1+2+3'), $typeName);
            $this->assertInternalType('string', $value);
            $this->assertEquals("1+2+3", $value,
    ...
    public function testAdapterQuoteTypeNonNumeric()
    {
        ...
        // test Zend_Db_Expr is passed through literally
        $value = $this->_db->quote(new Db\Expr('1+2+3'), 'CHAR');
        $this->assertInternalType('string', $value);
        $this->assertEquals("1+2+3", $value,
    ...
    public function testAdapterUpdateSetDbExpr()
    {
        ...
        $expr = new Db\Expr("UPPER('setExpr')");
        $rowsAffected = $this->_db->update(
            'zfbugs',
    ...
    public function testAdapterUpdateWhereDbExpr()
    {
        ...
        $whereExpr = new Db\Expr("$bug_id = 1");
    ...
    protected function _testAdapterAlternateStatement($stmtClass)
    {
        ...
        $this->assertInstanceOf($stmtClass, $stmt,
    ...
    public function testAdapterSetCustomProfiler()
    {
        ...
        try {
            ...
            $this->assertInstanceOf('Zend\Db\Profiler\Exception', $e);
    ...
    public function testAdapterSerializationIsOkByDefault()
    {
        ...
        $this->assertThat(unserialize($serialized), new \PHPUnit_Framework_Constraint_IsInstanceOf('Zend\Db\Adapter\AbstractAdapter'));
    }
    ...
    public function testAdapterSerializationFailsWhenNotAllowedToBeSerialized()
    {
        $params = $this->_util->getParams();
        $params['options'] = array(
            Db\Db::ALLOW_SERIALIZATION => false
        );
        $db = Db\Db::factory($this->getDriver(), $params);
    ...
    public function testAdapterUnSerializationAutoReconnection()
    {
        ...
        $params = $this->_util->getParams();
        $params['options'] = array(
            Db\Db::AUTO_RECONNECT_ON_UNSERIALIZE => true
        );
        $db = Db\Db::factory($this->getDriver(), $params);
