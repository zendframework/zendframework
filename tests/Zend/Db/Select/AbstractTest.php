...
abstract class AbstractTest extends \ZendTest\Db\TestSetup
{
    ...
    protected function _selectColumnsExpr()
    {
        ...
        $select = $this->_db->select()
            ->from('zfproducts', new Db\Expr($products.'.'.$product_name));
        return $select;
    }
    ...
    protected function _selectDistinctModifier()
    {
        $select = $this->_db->select()
            ->distinct()
            ->from('zfproducts', new Db\Expr(327));
        return $select;
    }
    ...
    protected function _selectWhereWithTypeFloat()
    {
        ...
        $select = $this->_db->select()
            ->from('zfprice')
            ->where("$price_total = ?", 200.45, Db\DB::FLOAT_TYPE);
        return $select;
    }
    ...
    protected function _selectGroupBy()
    {
        ...
        $select = $this->_db->select()
            ->from('zfbugs_products', array('bug_id', new Db\Expr("COUNT(*) AS $thecount")))
            ->group('bug_id')
            ->order('bug_id');
        return $select;
    }
    ...
    protected function _selectGroupByExpr()
    {
        ...
        $select = $this->_db->select()
            ->from('zfbugs_products', array('bug_id'=>new Db\Expr("$bug_id+1"), new Db\Expr("COUNT(*) AS $thecount")))
            ->group(new Db\Expr("$bug_id+1"))
            ->order(new Db\Expr("$bug_id+1"));
        return $select;
    }
    ...
    protected function _selectGroupByAutoExpr()
    {
        ...
        $select = $this->_db->select()
            ->from('zfbugs_products', array('bug_id'=>"ABS($bugs_products.$bug_id)", new Db\Expr("COUNT(*) AS $thecount")))
    ...
    {
        $this->setExpectedException('Zend\Db\Select\Exception');
