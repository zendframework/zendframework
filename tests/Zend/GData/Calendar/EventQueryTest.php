...
class EventQueryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->query = new Calendar\EventQuery();
    }
    ...
    {
        $this->query = new Calendar\EventQuery('http://www.foo.com');
        $queryUrl = $this->query->getQueryUrl();
    ...
    public function testUpdatedMinMaxParam()
    {
        ...
        $this->assertEquals(App\Util::formatTimestamp($updatedMin), $this->query->getUpdatedMin());
        $this->assertEquals(App\Util::formatTimestamp($updatedMax), $this->query->getUpdatedMax());
    ...
    {
       $testURI = "https://www.google.com/calendar/feeds/foo%40group.calendar.google.com/private/full";
       $this->query = new Calendar\EventQuery($testURI);
