 ...
 */
class CalendarFeedTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testEventFeedToAndFromString()
    {
        ...
        foreach ($this->listFeed as $entry) {
            $entryCount++;
            $this->assertTrue($entry instanceof Calendar\ListEntry);
        }
        $this->assertTrue($entryCount > 0);

        /* Grab XML from $this->listFeed and convert back to objects */
        $newListFeed = new Calendar\ListFeed(
    ...
    public function testSummary()
    {
        ...
        $entry = $feed[1];
        $this->assertTrue($entry->getSummary() instanceof Extension\Summary);
