 ...
 *
 * @category   Zend
 * @package    Zend\Http\Client\Cookies
 * @subpackage UnitTests
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 * @group      Zend_Http
...
class CookiesTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddCookie()
    {
        ...
        $this->assertTrue($cookie instanceof Cookie, '$cookie is expected to be a Cookie object');
    ...
    {
        $jar = new Cookies();
        $res_str = file_get_contents(dirname(__FILE__) . DIRECTORY_SEPARATOR . '..' .
        ...
        $this->assertEquals($cookie_str, $jar->getAllCookies(Cookies::COOKIE_STRING_CONCAT));
    }
    ...
    public function testGetAllCookiesAsConcat()
    {
        ...
        $real = $jar->getAllCookies(Cookies::COOKIE_STRING_CONCAT );
    ...
    {
        $cookie = Cookie::fromString('foo=bar; domain=www.example.com; path=/tests');
        $jar = new Cookies();
        $jar->addCookie($cookie->__toString(), 'http://www.example.com/tests/');

        $cobj = $jar->getCookie('http://www.example.com/tests/', 'foo');

        $this->assertTrue($cobj instanceof Cookie, '$cobj is not a Cookie object');
    ...
    public function testGetCookieAsString()
    {
        ...
        $cstr = $jar->getCookie('http://www.example.com/tests/', 'foo', Cookies::COOKIE_STRING_ARRAY);
        $this->assertEquals($cookie->__toString(), $cstr, 'Cookie string is not the expected string');

        $cstr = $jar->getCookie('http://www.example.com/tests/', 'foo', Cookies::COOKIE_STRING_CONCAT);
        $this->assertEquals($cookie->__toString(), $cstr, 'Cookie string is not the expected string');
    }
    ...
    public function testGetCookieReturnFalse()
    {
        ...
        $cstr = $jar->getCookie('http://www.example.com/tests/', 'otherfoo', Cookies::COOKIE_STRING_ARRAY);
        $this->assertFalse($cstr, 'getCookie was expected to return false, no such cookie');

        $cstr = $jar->getCookie('http://www.otherexample.com/tests/', 'foo', Cookies::COOKIE_STRING_CONCAT);
        $this->assertFalse($cstr, 'getCookie was expected to return false, no such domain');

        $cstr = $jar->getCookie('http://www.example.com/othertests/', 'foo', Cookies::COOKIE_STRING_CONCAT);
    ...
    {
        $cookie = Cookie::fromString('foo=bar; domain=example.com;');
        $jar = new Cookies();
        $jar->addCookie($cookie);
    ...
    public function testGetMatchingCookies($url, $expected)
    {
        $jar = new Cookies();
        $cookies = array(
            Cookie::fromString('foo1=bar1; domain=.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo2=bar2; domain=foo.com; path=/; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo3=bar3; domain=.foo.com; path=/; expires=' . date(DATE_COOKIE, time() - 3600)),
            Cookie::fromString('foo4=bar4; domain=.foo.com; path=/;'),
            Cookie::fromString('foo5=bar5; domain=.foo.com; path=/; secure; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo6=bar6; domain=.foo.com; path=/otherpath; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo7=bar7; domain=www.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo7=bar7; domain=newwww.foo.com; path=/;'),
            Cookie::fromString('foo8=bar8; domain=subdomain.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
        );

        foreach ($cookies as $cookie) $jar->addCookie($cookie);
        $cookies = $jar->getMatchingCookies($url);
        $this->assertEquals($expected, count($cookies), $jar->getMatchingCookies($url, true, Cookies::COOKIE_STRING_CONCAT));
    }
    ...
    public function testGetMatchingCookiesWithTime()
    {
        ...
        $cookies = array(
            Cookie::fromString('foo1=bar1; domain=.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo2=bar2; domain=.foo.com; path=/; expires=' . date(DATE_COOKIE, time() + 7200)),
            Cookie::fromString('foo3=bar3; domain=.foo.com; path=/; expires=' . date(DATE_COOKIE, time() - 3600)),
            Cookie::fromString('foo4=bar4; domain=.foo.com; path=/;'),
            Cookie::fromString('foo5=bar5; domain=.foo.com; path=/; secure; expires=' . date(DATE_COOKIE, time() - 7200)),
            Cookie::fromString('foo6=bar6; domain=.foo.com; path=/otherpath; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo7=bar7; domain=www.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
            Cookie::fromString('foo8=bar8; domain=subdomain.foo.com; path=/path; expires=' . date(DATE_COOKIE, time() + 3600)),
        );
        ...
        $cookies = $jar->getMatchingCookies('http://www.foo.com/path/file.txt', true, Cookies::COOKIE_OBJECT, time() + 3700);
    ...
    public function testGetMatchingCookiesAsStrings()
    {
        ...
        $cookies = $jar->getMatchingCookies('http://www.foo.com/path/file.txt', true, Cookies::COOKIE_STRING_ARRAY);
        ...
        $cookies = $jar->getMatchingCookies('http://www.foo.com/path/file.txt', true, Cookies::COOKIE_STRING_CONCAT);
    ...
    public function testExceptGetMatchingCookiesInvalidUri()
    {
        ...
        try {
            $cookies = $jar->getMatchingCookies('invalid.com', true, Cookies::COOKIE_STRING_ARRAY);
        ...
        try {
            $cookies = $jar->getMatchingCookies(new \stdClass(), true, Cookies::COOKIE_STRING_ARRAY);
            $this->fail('Expected getCookie to throw exception, invalid URI object passed');
        } catch (\Zend\Http\Exception $e) {
            // We are ok!
        }
    ...
    {
        $res_str = file_get_contents(dirname(realpath(__FILE__)) . DIRECTORY_SEPARATOR . '..' .
            DIRECTORY_SEPARATOR . '_files'  . DIRECTORY_SEPARATOR . 'response_with_single_cookie');
        $response = Response::fromString($res_str);

        $jar = Cookies::fromResponse($response, 'http://www.example.com');

        $this->assertTrue($jar instanceof Cookies, '$jar is not an instance of CookieJar as expected');
    ...
    public function testMatchPathWithTrailingSlash()
    {
        $jar = new Cookies();
        $cookies = array(
            Cookie::fromString('foo1=bar1; domain=.example.com; path=/a/b'),
            Cookie::fromString('foo2=bar2; domain=.example.com; path=/a/b/')
        );
    ...
    public function testIteratorAndCountable()
    {
        ...
        foreach ($jar as $cookie) {
            $this->assertInstanceOf('Zend\Cookie', $cookie);
        }
