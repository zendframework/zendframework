...
class TreeRouteStackTest extends TestCase
{
    ...
    public function testAssembleAbsoluteUriWithoutRequestUri()
    {
        $this->setExpectedException('Zend\Mvc\Router\Exception\RuntimeException', 'Request URI has not been set');
        $stack = new TreeRouteStack();
        
        $stack->addRoute('foo', new TestAsset\DummyRoute());
        $this->assertEquals('http://example.com:8080', $stack->assemble(array(), array('name' => 'foo', 'absolute' => true)));
    }
    
    public function testAssembleAbsoluteUriWithRequestUri()
    {
    ...
    public function testAssembleAbsoluteUriWithGivenUri()
    {
        $uri   = new HttpUri('http://example.com:8080/');
        $stack = new TreeRouteStack();
        
        $stack->addRoute('foo', new TestAsset\DummyRoute());
        $this->assertEquals('http://example.com:8080', $stack->assemble(array(), array('name' => 'foo', 'uri' => $uri, 'absolute' => true)));
    }
    
    public function testAssembleAbsoluteUriWithHostnameRoute()
    {
        $stack = new TreeRouteStack();
    ...
    public function testAssembleAbsoluteUriWithHostnameRouteWithoutScheme()
    {
        $this->setExpectedException('Zend\Mvc\Router\Exception\RuntimeException', 'Request URI has not been set');
        $stack = new TreeRouteStack();
        $stack->addRoute('foo', new Hostname('example.com'));
    ...
    public function testAssembleAbsoluteUriWithHostnameRouteAndRequestUriWithoutScheme()
    {
        $uri   = new HttpUri();
        $uri->setScheme('http');
        $stack = new TreeRouteStack();
